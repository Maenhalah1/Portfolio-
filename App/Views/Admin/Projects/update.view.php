...
<div class="admin-wrapper">
    ...
    <form action="" method="post" enctype="multipart/form-data">
        ...
        <input type="text" name="project_name" value="<?=static::inputValue("project_name", $project)?>"><br>
        <?=static::getFormErrors("project_name",$form_errors)?>

        <label for="project_link">Project Link:</label>
        <input type="text" name="project_link" value="<?=static::inputValue("project_link", $project)?>"><br>
        <?=static::getFormErrors("project_link",$form_errors)?>

        <label for="project_video_link">Project Video Link:</label>
        <input type="text" name="project_video_link" value="<?=static::inputValue("project_video_link", $project)?>"><br>
        <?=static::getFormErrors("project_video_link",$form_errors)?>

        <label for="project_description">Project Description:</label>
        <textarea name="project_description"><?=static::inputValue("project_description", $project)?></textarea><br>  
        <?=static::getFormErrors("project_description",$form_errors)?>


        <label for="client_name">Client Name:</label>
        <input type="text" name="client_name" value="<?=static::inputValue("client_name", $project)?>"><br>
